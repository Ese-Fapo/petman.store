        ...
        <>
                            ...
                            <td className="px-4 py-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <input type="checkbox" className="sr-only peer" onChange={() => toast.promise(toggleStock(product.id), {
                                        ...
                                        success: (message) => message || "Product stock updated",
                                        error: (error) => error.message || "Failed to update product stock",
            ...
            {editingProduct && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4 py-6">
                    <form onSubmit={(e) => toast.promise(updateProduct(e), {
                        ...
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                <h2 className="text-2xl text-slate-800 font-medium">Edit Product</h2>
                                <p className="mt-1 text-slate-400">Update the listing details customers see in the shop.</p>
                            </div>
                            <button
                                type="button"
                        ...
                        <p className="mt-2 text-xs text-slate-400">Leave images unchanged, or choose new images to replace the current set.</p>

                        <label className="mt-5 flex flex-col gap-2">
                            Name
                            <input type="text" name="name" onChange={handleEditChange} value={editForm.name} placeholder="Enter product name" className="w-full p-2 px-4 outline-none border border-slate-200 rounded" required />
                        </label>

                        <label className="mt-5 flex flex-col gap-2">
                            Description
                            <textarea name="description" onChange={handleEditChange} value={editForm.description} placeholder="Enter product description" rows={5} className="w-full p-2 px-4 outline-none border border-slate-200 rounded resize-none" required />
                        </label>

                        <div className="mt-5 grid gap-4 sm:grid-cols-2">
                            <label className="flex flex-col gap-2">
                                Actual Price
                                <input type="number" min={1} name="mrp" onChange={handleEditChange} value={editForm.mrp} placeholder="0" className="w-full p-2 px-4 outline-none border border-slate-200 rounded" required />
                            </label>
                            <label className="flex flex-col gap-2">
                                Offer Price
                                <input type="number" min={1} name="price" onChange={handleEditChange} value={editForm.price} placeholder="0" className="w-full p-2 px-4 outline-none border border-slate-200 rounded" required />
                            </label>
                        </div>

                        <select onChange={(e) => setEditForm({ ...editForm, category: e.target.value })} value={editForm.category} className="mt-5 w-full p-2 px-4 outline-none border border-slate-200 rounded" required>
                        ...
                        <div className="mt-7 flex flex-wrap gap-3">
                            <button disabled={saving} className="rounded bg-slate-800 px-6 py-2 text-white transition hover:bg-slate-900 disabled:cursor-not-allowed disabled:opacity-60">Save Changes</button>
                            <button type="button" onClick={closeEditProduct} className="rounded border border-slate-200 px-6 py-2 text-slate-600 transition hover:bg-slate-50">Cancel</button>
                        </div>
                    </form>
                </div>
            )}
